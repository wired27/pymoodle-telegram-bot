...
# Local in-memory refresh tracker; in production, consider persistent storage.
...
async def view_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        # Fetch the user's profile details from Moodle for additional info if needed.
        profile_text = (
            f"👤 <b>Name:</b> {user.first_name}\n"
            f"👤 <b>Surname:</b> {user.last_name}\n"
            f"🔑 <b>Telegram ID:</b> {telegram_id}\n"
            f"🔑 <b>Moodle Token:</b> {user.api_key}"
        )
    else:
        profile_text = "🔑 <b>You need to register your API token first.</b>"
    ...
    await update.callback_query.edit_message_text(profile_text, reply_markup=reply_markup, parse_mode="HTML")
...
async def get_grades(update: Update, context: ContextTypes.DEFAULT_TYPE, grade_type: str):
    ...
    if grades:
        ...
        for grade in grades:
            lines.append(
                ...
                f"📋 <b>{html.escape(grade_type.title())}:</b> <i>{html.escape(str(grade.get('assignment')))}</i>\n"
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    if len(message) > 4096:
        await update.callback_query.edit_message_text("🔍 Sending your grades in multiple messages...", parse_mode="HTML")
    else:
        await update.callback_query.edit_message_text(message, reply_markup=reply_markup, parse_mode="HTML")
...
async def show_notification_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Simple stub for notification settings.
    keyboard = [
        [InlineKeyboardButton("Set Deadline Reminders", callback_data="set_deadline_reminders")],
        [InlineKeyboardButton("Set Grades Reminders", callback_data="set_grades_reminders")],
        [InlineKeyboardButton("← Back", callback_data=CALLBACK_BACK_TO_MENU)]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.edit_message_text("🔔 Notification Settings:", reply_markup=reply_markup, parse_mode="HTML")
...
async def check_upcoming_deadlines(context: ContextTypes.DEFAULT_TYPE):
    ...
    user_service: UserService = context.application.bot_data["user_service"]
    moodle_service: MoodleService = context.application.bot_data["moodle_service"]
    telegram_ids = []  # Replace with actual logic to list all telegram_ids
